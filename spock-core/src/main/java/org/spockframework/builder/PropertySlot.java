...
public class PropertySlot implements ISlot {
  ...
  public Type getType() {
    ...
    if (setter != null) return ReflectionUtil.getResolvedParameterTypes(setter, ownerType).get(0);

    Field field = MopUtil.fieldFor(property);
    if (field != null) return ReflectionUtil.getResolvedFieldType(field, ownerType);
